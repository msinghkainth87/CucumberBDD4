...
public class DataCompareTransactionNew {
...
//                        System.out.println("Target : " + key1 + " Value" + value);
                        if (key1.contains(mapMappingData.get("Input_File_Field"))) {
                            ...
                            System.out.println("TargetSheet"+mapTarget.get("FIXED_RATE"));


                            System.out.println("SourceLegTarget"+Float.parseFloat(mapSource.get("FIXED_RATE"))*100);
                            //System.out.println("TargetNotional"+mapTarget.get("TARGET_NOTNL_AMT"));

//                            if(Character.getNumericValue(mapSource.get("TARGET_NOTNL_AMT"))==Character.getNumericValue(mapTarget.get("TARGET_NOTNL_AMT")))
...
//                                System.out.println("ELse " + mapTran.get('"'+srcKey+'"').replaceAll("\"",""));
                ...
                if(mapResult.size()>1) {
                    exception = exception + "\n"+ "\n" + "Mismatch at Row :" + mapResult.get("Row") + " " + "for "+id+" : "+ TradeId + " " + mapSource.get("SEC_TYP_CD")+ " "+ mismatchList;
                }
